...
const departmentsList = ref(contacts);
...
// Computed properties
const displayedPosts = computed(() => paginate(departmentsList.value));
...
async function handleSubmit() {
  ...
  const isValid =
    event.value.department_name;
  if (isValid) {
    if (dataEdit.value) {
      if (isFormValid()) {
        addDepartmentModal.value = false;
        departmentsList.value = departmentsList.value.map(item => item.id.toString() === event.value.id.toString() ? { ...item, ...event.value } : item);
        try {
          const response = await axios.post('/api/departments/store', event.value);
      ...
      if (isFormValid()) {
        ...
        try {
          const response = await axios.post('/api/departments/store', data);
          console.log('Department submitted successfully:', response.data);
        } catch (error) {
          console.error('Error submitting department:', error);
        }
        departmentsList.value = [data, ...departmentsList.value];
        addDepartmentModal.value = false;
      }
    }
  }
...
async function deleteData() {
  if (event.value.id) {
    departmentsList.value = departmentsList.value.filter((item) => item.id !== event.value.id);
    deleteModal.value = false;
    loading.value = true;
    error.value = null;
    try {
      const response = await axios.delete(`/api/departments/${event.value.id}`)
      // const response = await axios.post('/api/leads/delete');  // Adjust the API endpoint as needed
      console.log(response);
      departmentsList.value = response.data.result;
    } catch (err) {
...
function showdetail(data: any) {
  const id = data.id;
  Inertia.visit(`/apps/department-profile/`+id);
}

function setPages() {
  const numberOfPages = Math.ceil(departmentsList.value.length / perPage.value);
  pages.value = [];
...
function deleteMultiple() {
  ...
  if (ids_array.length > 0) {
    Swal.fire({
      ...
      if (result.value) {
        departmentsList.value = departmentsList.value.filter((customer) => !ids_array.includes(customer.id));
...
const fetchdepartments = async () => {
  ...
  try {
    ...
    response.data.result.forEach(department => {
      if (department.tags) {
        department.tags = department.tags.split(',').map(tag => tag.trim());
      }
    });
    departmentsList.value = response.data.result;
  } catch (err) {
...
const getImageSrc = (img: string | null) => {
  ...
  return img ? new URL(`@/images/departments/${img}`, import.meta.url).href : new URL('@/images/avatar-1.jpg', import.meta.url).origin+"/resources/images/users/avatar-1.jpg";
};
</script>
...
<template>
  <Layout>
    ...
    <PageHeader title="Department" pageTitle="HR" />
    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <BCardHeader>
      ...
      <BCol xxl="12">
        <BCard no-body id="departmentList">
          <BCardHeader>
            <BRow class="g-3">
              <BCol md="4">
                <div class="search-box">
                  <input type="text" class="form-control search" placeholder="Search for department..." v-model="searchQuery" />
                    ...
                    <tr v-for="(data, index) of resultQuery" :key="index">
                      ...
                      <td class="">
                        <div class="d-flex align-items-center">
                          ...
                          <div class="flex-grow-1 ms-2 name">{{ data.department_name }}</div>
                        </div>
                      </td>
                      <td>
    ...
    <BModal v-model="addDepartmentModal" id="showmodal" hide-footer title-class="exampleModalLabel" header-class="bg-info-subtle p-3" class="v-modal-custom" centered :title="dataEdit ? 'Edit Department' : 'Add Department'">
      <BForm action="" id="addform" class="tablelist-form" autocomplete="off">
        ...
        <BRow class="g-3">
          ...
          <BCol lg="12">
            <div>
              <label for="email_id-field" class="form-label">Department Id</label>
              <input type="text" id="empid" class="form-control" placeholder="Enter department id" v-model="event.empid" />
        ...
        <div class="hstack gap-2 justify-content-end mt-3">
          <BButton type="button" variant="light" @click="addDepartmentModal = false" id="edtclosemodal">Close</BButton>
          <BButton type="button" variant="success" id="add-btn" @click="handleSubmit">
            {{ dataEdit ? 'Update' : 'Add Department' }}
          </BButton>
        </div>
      </BForm>
    </BModal>
    ...
    <BModal v-model="deleteModal" modal-class="zoomIn" hide-footer no-close-on-backdrop centered>
      <div class="mt-2 text-center">
        ...
        <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
          ...
          <p class="text-muted mx-4 mb-0">Deleting your department will remove all of your information from our database.</p>
        </div>
      </div>

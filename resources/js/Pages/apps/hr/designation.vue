...
const addDesignationModal = ref(false);
const page = ref(1);
const perPage = ref(8);
...
const designationsList = ref(contacts);
...
// Computed properties
const displayedPosts = computed(() => paginate(designationsList.value));
...
async function handleSubmit() {
  ...
  const isValid =
    event.value.designation_name;
  if (isValid) {
    if (dataEdit.value) {
      if (isFormValid()) {
        addDesignationModal.value = false;
        designationsList.value = designationsList.value.map(item => item.id.toString() === event.value.id.toString() ? { ...item, ...event.value } : item);
        try {
          const response = await axios.post('/api/designations/store', event.value);
      ...
      if (isFormValid()) {
        ...
        try {
          const response = await axios.post('/api/designations/store', data);
          console.log('Designation submitted successfully:', response.data);
        } catch (error) {
          console.error('Error submitting designation:', error);
        }
        designationsList.value = [data, ...designationsList.value];
        addDesignationModal.value = false;
      }
    }
  }
...
function isFormValid() {
  return event.value.designation_name
}

function onSort(column: keyof typeof event.value) {
  const direction = ref<'asc' | 'desc'>('asc');
  direction.value = direction.value === 'asc' ? 'desc' : 'asc';
  const sortedArray = [...designationsList.value];
  sortedArray.sort((a, b) => {
    const res = a[column] < b[column] ? -1 : a[column] > b[column] ? 1 : 0;
    return direction.value === 'asc' ? res : -res;
  });
  designationsList.value = sortedArray;
}

function editDetails(data: any) {
  addDesignationModal.value = true;
  dataEdit.value = true;
  event.value = {
    id: data.id,
    image: data.image,
...
async function deleteData() {
  if (event.value.id) {
    designationsList.value = designationsList.value.filter((item) => item.id !== event.value.id);
    deleteModal.value = false;
    loading.value = true;
    error.value = null;
    try {
      const response = await axios.delete(`/api/designations/${event.value.id}`)
      // const response = await axios.post('/api/leads/delete');  // Adjust the API endpoint as needed
      console.log(response);
      designationsList.value = response.data.result;
    } catch (err) {
...
function showdetail(data: any) {
  const id = data.id;
  Inertia.visit(`/apps/designation-profile/`+id);
}

function setPages() {
  const numberOfPages = Math.ceil(designationsList.value.length / perPage.value);
  pages.value = [];
...
function deleteMultiple() {
  ...
  if (ids_array.length > 0) {
    Swal.fire({
      ...
      if (result.value) {
        designationsList.value = designationsList.value.filter((customer) => !ids_array.includes(customer.id));
...
const fetchdesignations = async () => {
  ...
  error.value = null;
  try {
    const response = await axios.get('/api/designations');  // Adjust the API endpoint as needed
    console.log(response);
    response.data.result.forEach(designation => {
      if (designation.tags) {
        designation.tags = designation.tags.split(',').map(tag => tag.trim());
      }
    });
    designationsList.value = response.data.result;
  } catch (err) {
    error.value = 'Failed to fetch designations data.';
    console.error(err);
  } finally {
    loading.value = false;
  }
...
const fetchDepartments = async () => {
  ...
  error.value = null;
  try {
    const response = await axios.get('/api/departments');  // Adjust the API endpoint as needed
    console.log(response);
    departmentOptions.value = 
      response.data.result.map((department: any) => ({
        value: department.id,
        text: department.department_name
      }));
    console.log(departmentOptions);
  } catch (err) {
    error.value = 'Failed to fetch departments data.';
    console.error(err);
  } finally {
    loading.value = false;
  }
...
const getImageSrc = (img: string | null) => {
  ...
  return img ? new URL(`@/images/designations/${img}`, import.meta.url).href : new URL('@/images/avatar-1.jpg', import.meta.url).origin+"/resources/images/users/avatar-1.jpg";
};
</script>
...
<template>
  <Layout>
    ...
    <PageHeader title="Designation" pageTitle="HR" />
    <BRow>
      <BCol lg="12">
        <BCard no-body>
          <BCardHeader>
            <div class="d-flex align-items-center flex-wrap gap-2">
              <div class="flex-grow-1">
                <BButton variant="info" class="add-btn me-1" @click="toggleModal">
                  <i class="ri-add-fill me-1 align-bottom"></i> Add Designation
                </BButton>
      ...
      <BCol xxl="12">
        <BCard no-body id="designationList">
          <BCardHeader>
            <BRow class="g-3">
              <BCol md="4">
                <div class="search-box">
                  <input type="text" class="form-control search" placeholder="Search for designation..." v-model="searchQuery" />
                ...
                <table class="table align-middle table-nowrap mb-0" id="customerTable">
                  <thead class="table-light">
                    <tr>
                      ...
                      <th class="sort" data-sort="name" scope="col" @click="onSort('name')">Department</th>
                    ...
                    <tr v-for="(data, index) of resultQuery" :key="index">
                      ...
                      <td class="">
                        <div class="d-flex align-items-center">
                          <div class="flex-grow-1 ms-2 name">{{ data.department_id }}</div>
                        </div>
                      </td>
                      <td class="">
                        <div class="d-flex align-items-center">
                          ...
                          <div class="flex-grow-1 ms-2 name">{{ data.designation_name }}</div>
                        </div>
                      </td>
                      <td>
    ...
    <BModal v-model="addDesignationModal" id="showmodal" hide-footer title-class="exampleModalLabel" header-class="bg-info-subtle p-3" class="v-modal-custom" centered :title="dataEdit ? 'Edit Designation' : 'Add Designation'">
      <BForm action="" id="addform" class="tablelist-form" autocomplete="off">
        ...
        <BRow class="g-3">
          ...
          <BCol lg="12">
            <div>
              ...
              <BFormSelect v-model="event.department_name" class="mb-3" aria-label="Default select example" :options="departmentOptions" :close-on-select="true" :searchable="true" :create-option="false">
              </BFormSelect>
        ...
        <div class="hstack gap-2 justify-content-end mt-3">
          <BButton type="button" variant="light" @click="addDesignationModal = false" id="edtclosemodal">Close</BButton>
          <BButton type="button" variant="success" id="add-btn" @click="handleSubmit">
            {{ dataEdit ? 'Update' : 'Add Designation' }}
          </BButton>
        </div>
      </BForm>
    </BModal>
    ...
    <BModal v-model="deleteModal" modal-class="zoomIn" hide-footer no-close-on-backdrop centered>
      <div class="mt-2 text-center">
        ...
        <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
          <h4>You are about to delete a designation ?</h4>
          <p class="text-muted mx-4 mb-0">Deleting your designation will remove all of your information from our database.</p>
        </div>
      </div>
